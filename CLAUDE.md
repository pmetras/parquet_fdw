...
INT8/INT16→INT2, INT32→INT4, INT64→INT8, UINT8→INT2, UINT16→INT4, UINT32→INT8, UINT64→NUMERIC, FLOAT→FLOAT4, DOUBLE→FLOAT8, TIMESTAMP→TIMESTAMP, DATE32→DATE, STRING/LARGE_STRING→TEXT, BINARY/LARGE_BINARY→BYTEA, LIST/LARGE_LIST→ARRAY, MAP→JSONB, UUID→UUID, FIXED_SIZE_BINARY(16)→UUID
